...
class DiscretetizedIntegratedGradients(GradientAttribution):
	...
	def attribute(
		...
		attributions, interpolated_input, gradientsAt_interpolation, cummulative_gradients = self.calculate_dig_attributions(scaled_features_tpl=scaled_features_tpl, target=target, additional_forward_args=additional_forward_args, n_steps=n_steps)
		...
		if return_convergence_delta:
			...
			return _format_output(is_inputs_tuple, attributions), delta, interpolated_input, gradientsAt_interpolation, cummulative_gradients

		return _format_output(is_inputs_tuple, attributions), interpolated_input, gradientsAt_interpolation, cummulative_gradients
	...
	def calculate_dig_attributions(
		...
		cummulative_gradients = tuple(_reshape_and_sum(scaled_grad, 1, grad.shape[0] // 1, grad.shape[1:]) for (scaled_grad, grad) in zip(scaled_grads, grads))
		...
		#return attributions
		return attributions,  scaled_features_tpl[0], scaled_grads[0], cummulative_gradients[0]
